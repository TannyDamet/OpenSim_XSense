...
%% Ajoutez ici l'importation de la bibliothèque de filtre de Kalman si nécessaire

%% 


%% Appliquer le filtre de Kalman aux données de chaque capteur

% Définir les matrices de covariance de bruit de mesure et de bruit de processus
% (Vous devrez ajuster les valeurs de ces matrices en fonction de votre cas d'utilisation)
R_measurement = eye(3); % Matrice de covariance de bruit de mesure
Q_process = eye(3);     % Matrice de covariance de bruit de processus

% Initialiser les états estimés et les matrices de covariance pour chaque capteur
x_est_1 = [0; 0; 0]; % État initial estimé pour le capteur 1
P_1 = eye(3);       % Matrice de covariance initiale pour le capteur 1

x_est_2 = [0; 0; 0]; % État initial estimé pour le capteur 2
P_2 = eye(3);       % Matrice de covariance initiale pour le capteur 2

x_est_3 = [0; 0; 0]; % État initial estimé pour le capteur 3
P_3 = eye(3);       % Matrice de covariance initiale pour le capteur 3

x_est_4 = [0; 0; 0]; % État initial estimé pour le capteur 4
P_4 = eye(3);       % Matrice de covariance initiale pour le capteur 4

x_est_5 = [0; 0; 0]; % État initial estimé pour le capteur 5
P_5 = eye(3);       % Matrice de covariance initiale pour le capteur 5

%%
% Récupération des données 

% data1 = open("Tanny_Calib + D2\Xsens_1_20201125_110809_692.csv");
% data2 = open("Tanny_Calib + D2\Xsens_2_20201125_110809_693.csv");
% data3 = open("Tanny_Calib + D2\Xsens_3_20201125_110809_690.csv");
% data4 = open("Tanny_Calib + D2\Xsens_4_20201125_110809_691.csv");
% data5 = open("Tanny_Calib + D2\Xsens_5_20201125_110809_694.csv");

data1 = open("Tanny_Calib + D2\Xsens_1.csv");
data2 = open("Tanny_Calib + D2\Xsens_2.csv");
data3 = open("Tanny_Calib + D2\Xsens_3.csv");
data4 = open("Tanny_Calib + D2\Xsens_4.csv");
data5 = open("Tanny_Calib + D2\Xsens_5.csv");
...
for i = 1:taille
    ...
    % Récupérer les états mesurés de chaque capteur pour l'itération i
    measurements_1 = data1.data(i, 3:9)';
    measurements_2 = data2.data(i, 3:9)';
    measurements_3 = data3.data(i, 3:9)';
    measurements_4 = data4.data(i, 3:9)';
    measurements_5 = data5.data(i, 3:9)';

    % Mise à jour des états estimés et des matrices de covariance pour chaque capteur
    [x_est_1, P_1] = kalmanFilter(measurements_1, x_est_1, P_1, R_measurement, Q_process);
    [x_est_2, P_2] = kalmanFilter(measurements_2, x_est_2, P_2, R_measurement, Q_process);
    [x_est_3, P_3] = kalmanFilter(measurements_3, x_est_3, P_3, R_measurement, Q_process);
    [x_est_4, P_4] = kalmanFilter(measurements_4, x_est_4, P_4, R_measurement, Q_process);
    [x_est_5, P_5] = kalmanFilter(measurements_5, x_est_5, P_5, R_measurement, Q_process);

    % Mettre à jour les valeurs des accélérations dans les matrices de données
    data1.data(i, 3:9) = x_est_1';
    data2.data(i, 3:9) = x_est_2';
    data3.data(i, 3:9) = x_est_3';
    data4.data(i, 3:9) = x_est_4';
    data5.data(i, 3:9) = x_est_5';
    
    quats_1 = x_est_1(i,1:4);
    quats_2 = x_est_2(i,1:4);
    quats_3 = x_est_3(i,1:4);
    quats_4 = x_est_4(i,1:4);
    quats_5 = x_est_5(i,1:4);
    ...
    old_acc1 = x_est_1(i,5:7)';
    old_acc2 = x_est_2(i,5:7)';
    old_acc3 = x_est_3(i,5:7)';
    old_acc4 = x_est_4(i,5:7)';
    old_acc5 = x_est_5(i,5:7)';
...
writecell(data1_modified, "Tanny_Calib + D2\Xsens_1_modified.csv");
writecell(data2_modified, "Tanny_Calib + D2\Xsens_2_modified.csv");
writecell(data3_modified, "Tanny_Calib + D2\Xsens_3_modified.csv");
writecell(data4_modified, "Tanny_Calib + D2\Xsens_4_modified.csv");
writecell(data5_modified, "Tanny_Calib + D2\Xsens_5_modified.csv");
...
function [x_est, P] = kalmanFilter(measurements, x_est_prev, P_prev, R, Q)
    % Mesures (accélérations) au temps courant
    y = measurements;
    ...
    % Calcul de la résiduelle
    residual = y - x_pred;

    % Calcul de la matrice de gain de Kalman
    K = P_pred / (P_pred + R);

    % Mise à jour de l'état estimé et de la matrice de covariance
